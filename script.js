const message = {
    ...
    randPlace() {
        let randIndex = Math.floor(Math.random() * (this.places.length));
        return this.places[randIndex]
    },
    randActivity() {
        let randIndex = Math.floor(Math.random() * (this.activities.length));
        return this.activities[randIndex]
    },
    randEnding() {
        let randIndex = Math.floor(Math.random() * (this.endingIdeas.length));
